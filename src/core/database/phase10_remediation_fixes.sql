-- ==============================================================================
-- NEGOCIO FLEX - FASE 10: REMEDIACIÓN FORENSE DE SEGURIDAD Y CONCURRENCIA
-- Corrige:
-- 1. Bypass de Límites mediante REST / INSERT directo (Triggers y Locks)
-- 2. Activación de Suscripción sin Pago Real (Flujo PENDING)
-- 3. Eliminación de p_signature_valid en RPCs
-- 4. Protección contra Concurrencia (pg_advisory_xact_lock y SELECT FOR UPDATE)
-- 5. Protección contra Webhooks Fuera de Orden (Timestamp comparison)
-- 6. Reintentos de Webhook FAILED (FAILED -> RETRY -> PROCESSED)
-- 7. Máquina de Estados Enforced en PostgreSQL (Trigger BEFORE UPDATE)
-- ==============================================================================

-- 1. FUNCIÓN Y TRIGGERS DE ENFORCEMENT DE LÍMITES EN BASE DE DATOS
-- ------------------------------------------------------------------------------
...
BEGIN
  ...
  -- 1.1. Serialización concurrente mediante advisory lock por organización y tabla
  -- Previene carreras cuando dos transacciones intentan insertar simultáneamente en el límite
  PERFORM pg_advisory_xact_lock(hashtext('org_limit_' || v_org_id::text || '_' || TG_TABLE_NAME));

  -- 1.2. Obtener la suscripción activa o de prueba del tenant
  SELECT * INTO v_sub
  ...
  -- 1.3. Contar registros actuales y verificar cuota
  CASE TG_TABLE_NAME
    WHEN 'products' THEN
  ...
  -- 1.4. Bloqueo atómico si se supera la cuota permitida
...
-- Instalar Triggers de Límites en todas las tablas restringidas
...
-- 2. ENFORCEMENT DE MÁQUINA DE ESTADOS EN POSTGRESQL (TRIGGER BEFORE UPDATE)
-- ------------------------------------------------------------------------------
...
-- 3. REMEDIACIÓN: UPGRADE SEGURO SIN INVENTAR TRANSACCIONES APROBADAS
-- ------------------------------------------------------------------------------
...
BEGIN
  ...
  -- 4. Obtener datos del plan objetivo desde el catálogo oficial en el servidor
  ...
  -- 5. Bloqueo de fila concurrente en la suscripción existente
  ...
  -- NOTA DE SEGURIDAD: NO se actualiza la suscripción a 'active' aquí.
...
-- 4. REMEDIACIÓN: WEBHOOK SEGURO SIN p_signature_valid, CON LOCKS Y ANTI-OUT-OF-ORDER
-- ------------------------------------------------------------------------------
...
BEGIN
  ...
  BEGIN
    IF p_event_type = 'payment.succeeded' OR p_event_type = 'charge.successful' THEN
      ...
      -- Actualizar cualquier transacción pendiente previa a APPROVED
    ...
    ELSIF p_event_type = 'payment.failed' OR p_event_type = 'charge.failed' THEN
      ...
      IF v_target_org_id IS NOT NULL THEN
        ...
        IF v_sub.id IS NOT NULL AND v_sub.status = 'active' AND v_sub.last_reconciled_at > v_event_timestamp THEN
          -- Out-of-order event detectado: no degradar
          UPDATE public.webhook_logs
...
-- REVOCAR ACCESO DIRECTO PÚBLICO Y CONCEDER EXCLUSIVAMENTE A SERVICE_ROLE
...
-- 5. CANCELACIÓN CON BLOQUEO PESIMISTA
-- ------------------------------------------------------------------------------
